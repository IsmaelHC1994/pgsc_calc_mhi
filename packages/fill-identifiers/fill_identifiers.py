#!/usr/bin/env python3
"""
fill_identifiers.py — Batch-fill patient identifiers in generated DOCX reports.
...
then replaces every occurrence of the placeholder text with the real values.
DOCX files also get the footer when python-docx is installed.
...
def find_report_files(reports_dir, sample_id):
    ...
    for root, _dirs, files in os.walk(reports_dir):
        for f in files:
            if f.startswith(pattern) and f.endswith(".docx"):
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch-fill patient identifiers in generated DOCX reports."
    )
    parser.add_argument(
        "--csv", required=True,
    ...
    parser.add_argument(
        ...
        help="Root directory containing generated DOCX report files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Directory for untouched original backups. Default: sibling of --reports-dir named <reports-dir>_original_backups."
    )
    ...
    print(f"Searching for DOCX reports in: {args.reports_dir}")
    ...
    if args.backup_dir:
        ...
        reports_abs = os.path.abspath(args.reports_dir)
        backup_dir = f"{reports_abs}_original_backups"
